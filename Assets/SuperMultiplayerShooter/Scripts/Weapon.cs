...
namespace Visyde
{
    ...
    public class Weapon : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!gm.isGameOver && !gm.ui.isMenuShown)
            {
                if (owner.photonView.isMine)
                {
                    ...
                    if (owner.shooting)
                    {
                        if (curFR >= 1)
                        {
                            ...
                            if (curAmmo > 0)
                            {
                                PhotonNetwork.RPC(owner.photonView, "Shoot", PhotonTargets.All, true, owner.mousePos, owner.movementController.rg.position);
                            }
                            else
                            {
        ...
        public void Shoot()
        {
            ...
            {
                Projectile p = owner.pooler.Spawn(projectile, shootPoint.position, transform.rotation).GetComponent<Projectile>();

                // Let the projectile know what "x" direction to face:
                int xDir = owner.transform.localScale.x < 0? -1 : 1;

                // Setups:
                p.Reset(projectileSpeed, projectileAcceleration, xDir, projectileLifetime, destroyProjectileWhenOwnerDies, owner, owner.lastWeaponId, owner.pooler);
                p.RefreshLastPos();
                // Projectile angle:
