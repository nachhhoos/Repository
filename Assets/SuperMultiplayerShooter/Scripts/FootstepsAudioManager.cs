...
namespace Visyde
{
    ...
    public class FootstepsAudioManager : MonoBehaviour
    {
        ...
        public void Step()
        {
            ...
            player.aus.PlayOneShot(player.characters[player.curCharacter].data.footstepSFX[Random.Range(0, player.characters[player.curCharacter].data.footstepSFX.Length)]);
        }
    }
}