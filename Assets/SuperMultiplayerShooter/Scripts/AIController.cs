...
namespace Visyde
{
    ...
    public class AIController : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            Vector3 bounds = player.gm.maps[player.gm.chosenMap].bounds;
            Vector3 offset = player.gm.maps[player.gm.chosenMap].boundOffset;
        ...
        void Update()
        {
            if (player.gm.gameStarted && !player.gm.isGameOver && player.photonView.isMine)
            {
                if (player)
                {
                    ...
                    if (!player.gm.isGameOver)
                    {
                        ...
                        if (nearestPlayer)
                        {
                            ...
                            if (chosenEmote < 0) chosenEmote = Random.Range(0, 2);
                        }
                        else
                        {
                            if (wanderDelay <= 0)
                ...
                {
                    print("<color=yellow>(AI Controller)</color> Please attach a PlayerController component to work with.");
                }
            }
        ...
        void GetNearestPlayer()
        {
            ...
            if (cols.Length > 0)
            {
                ...
                // Iterate through the acquired players list and get the nearest one:
                for (int i = 0; i < cols.Length; i++)
                {
                    if (cols[i].CompareTag("Player"))
                    {
                        ...
                        {
                            PlayerController curP = cols[i].GetComponent<PlayerController>();
                            // Ignore if invulnerable:
                            if (!curP.invulnerable)
                            {
                                if (!nearestPlayer)
                                {
                                    // Set the first player in the list as the nearest (will be replaced by nearer players later down the loop):
                                    nearestPlayer = curP;
                                    nearestDist = Helper.GetDistance(transform.position, curP.transform.position);
                                }
                                else
                                {
                                    // Get the distance of the current player from us and compare it to the last one:
                                    float curDist = Helper.GetDistance(transform.position, curP.transform.position);
        ...
        void DoMove(Vector3 destination)
        {
            ...
            else
            {
                ...
                if (!leftPlatform)
                {
                    leftPlatform = player.movementController.rg.velocity.y <= 0 && !SideHasGround(Mathf.RoundToInt(doMoveDir) * -1) && transform.position.y >= (lastFoundPlatformHeight.HasValue ? lastFoundPlatformHeight.Value : transform.position.y);
                }
        ...
        IEnumerator DoEmote()
        {
            while (true)
            {
                ...
                if (chosenEmote > 0)
                {
                    player.photonView.RPC("Emote", PhotonTargets.All, 0);
                    chosenEmote = -1;
                }
