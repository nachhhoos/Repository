    ...
    public class Projectile : MonoBehaviour
    {
        ...
        void CheckHit()
        {
            ...
            if (!shot){
                if (hit){
                    if (!hit.collider.CompareTag("Player"))
                    {
                        ...
                        if (!p || p.GetOwner().NickName != ownerName){
                            shot = true;
                        }
                    }
                }
            ...
            if (shot)
            {
                if (hit && !destroyNow)
                {
                    // Let projectile pass through pickups:
                    if (!hit.collider.CompareTag("Pickup"))
                    {
                        ...
                        if (explosionRadius > 0)
                        {
                            ...
                            // VFX
                            pooler.Spawn(explosionVFX, hit.point, Quaternion.identity);
                        ...
                        else
                        {
                            ...
                            if (hit.collider.CompareTag("Player"))
                            {
                                ...
                                if (p)
                                {
                                    PhotonPlayer pp = p.GetOwner();
                                    // Don't do anything on owner hit:
                                    if (pp.NickName != ownerName)
                                    {
                                        // Do only vfx if invulnerable:
                                        if (p.invulnerable)
                                        {
                                            // VFX
                                            pooler.Spawn(shieldHitVFX, hit.point, Quaternion.identity);
                                        }
                                        // Do vfx and damage if not:
                                        else
                                        {
                                            if (thisIsMine) p.photonView.RPC("Hurt", PhotonTargets.All, ownerName, weaponId, true);
                                            // VFX
                                            pooler.Spawn(bodyHitVFX, hit.point, Quaternion.identity);
                                        }
        ...
        void Explode()
        {
            ...
            for (int i = 0; i < cols.Length; i++)
            {
                if (cols[i].CompareTag("Player"))
                {
                    ...
                    if (((p.photonView.owner == owner.photonView.owner && GameManager.instance.allowHurtingSelf) || p.photonView.owner != owner.photonView.owner) && !p.invulnerable)
                    {
                        if (!p.isDead)
                        {
                            ...
                            for (int h = 0; h < hits.Length; h++)
                            {
                                if (hits[h].collider.gameObject == cols[i].gameObject)
                                {
                                    ...
                                    // Apply damage:
                                    p.photonView.RPC("Hurt", PhotonTargets.AllBuffered, ownerName, finalDamage, false);
                                    break;
                                }
                            }
                        }
        ...
        /// </summary>
        public void Reset(float projSpeed, float acceleration, int xDirection, float projectileLifetime, bool destroyWhenOwnerDies, PlayerController from, int idOfWeapon, ObjectPooler objectPooler)
        {
            speed = projSpeed;
            ...
            // other settings:
            ownerName = owner.GetOwner().NickName;
            thisIsMine = owner.photonView.isMine;
