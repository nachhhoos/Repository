...
namespace Visyde
{
    ...
    public class PowerUpPickup : PunBehaviour
    {
        [Header("References:")]
            ...
            if (allowPickup)
            {
                if (col.tag == "Player")
                {
                    ...
                    if (p)
                    {
                        ...
                        {
                            p.photonView.RPC("ReceivePowerUp", PhotonTargets.AllViaServer, (int)data[0], (int)data[1]);
                            photonView.RPC("Picked", PhotonTargets.All);
