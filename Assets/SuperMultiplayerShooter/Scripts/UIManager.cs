...
namespace Visyde
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        [HideInInspector] public bool isMenuShown;
        ...
        void Update()
        {
            ...
            isMenuShown = menuPanel.activeSelf;
            ...
            else
            {
                ...
                if (gm.gameStarted)
                {
                    ...
                    // Tick:
                    if (curRemainingTimeTick + 1 >= (gm.gameLength - gm.elapsedTime))
                    {
        ...
        /// <summary>
        /// Updates the scoreboard's and leaderboard's contents.
        /// </summary>
        public void UpdateBoards()
        {
            ...
            if (!gm.isGameOver)
            {
                PhotonPlayer[] playersSorted = gm.SortPlayersByScore();

                //print("Players: " + playersSorted.Length);
                //print(" | " + (int)playersSorted[0].CustomProperties["kills"]);
                //print(" | " + (int)playersSorted[1].CustomProperties["kills"]);
                //print(" | " + (int)playersSorted[0].CustomProperties["kills"]);
                ...
                {
                    kills[i] = (int)playersSorted[i].CustomProperties["kills"];
                    deaths[i] = (int)playersSorted[i].CustomProperties["deaths"];
                    otherScore[i] = (int)playersSorted[i].CustomProperties["otherScore"];
                }
                ...
                for (int i = 0; i < leaderboardItems.Length; i++)
                {
                    ...
                    {
                        leaderboardItems[i].playerName.text = playersSorted[i].NickName;
                        leaderboardItems[i].playerName.color = leaderboardItems[i].playerName.text == PhotonNetwork.playerName ? Color.cyan : Color.white;
        ...
        public void SomeoneKilledSomeone(string dying, string killer)
        {

            // You we're killed:
            if (dying == PhotonNetwork.player.NickName)
            {
                ...
                {
                    DisplayMessage("You've been killed by " + killer, MessageType.Death);
                }
                // Die sound effect:
            ...
            {
                // You killed someone:
                if (killer == PhotonNetwork.player.NickName)
                {
                    DisplayMessage("You killed " + dying + "!", MessageType.Kill);
                    // Kill sound effect:
                ...
                else
                {
                    ...
                    if (killer == dying)
                    {
                        DisplayMessage(dying + " committed suicide", MessageType.Normal);
                    }
                    // Someone killed someone:
                    else
                    {
                        DisplayMessage(killer + " killed " + dying, MessageType.Normal);
                    }
                }
            }
